...
import com.amazonaws.services.kinesis.samples.streams.model.*;
...
public class StockOrderTradeGenerator {
    ...
    private  AtomicLong orderIdSeq = new AtomicLong(3000001);
    private  AtomicLong tradeIdSeq = new AtomicLong(5000001);
    ...
    public StockOrder getRandomStock() {
        // pick a random stock
        ...
        StockOrder.TradeType tradeType = StockOrder.TradeType.BUY;
        if (random.nextDouble() < PROBABILITY_SELL) {
            tradeType = StockOrder.TradeType.SELL;
        }
        ...
        StockTrade trade = new StockTrade(stockPrice.tickerSymbol, orderId, tradeIdSeq.getAndIncrement(), amount, price, quantity);
        
        order.setStockTrade(trade);
